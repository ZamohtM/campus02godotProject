...
public partial class ShoppingListContext : Control
{
	...
	public override async void _Ready()
	{
		...
		connector = GlobalData.Instance.connector;
		...
		string response = await connector._get_shoppinglist_request(GlobalData.Instance.openedShoppingListId, GetNode<HttpRequest>("HTTPShoppingList"));
	...
	private async void _on_save_button_pressed()
	{
		...
		string response = await connector._patch_shoppinglist_request(GlobalData.Instance.openedShoppingListId,GlobalData.Instance.opendShoppingList,GetNode<HttpRequest>("HTTPShoppingListProducts"));
	...
	{
		string response = await connector._delete_shoppinglist_request(GlobalData.Instance.openedShoppingListId,GetNode<HttpRequest>("HTTPShoppingListDelete"));
		if(response.Equals("200"))
		{
			GetTree().ChangeSceneToFile("res://scenes/MainContext.tscn");
		}
